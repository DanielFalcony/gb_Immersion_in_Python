...
def group_rename(count_nums_name: int, old_file_ext: str, new_file_ext: str, name_range: list,
    ...
    for dir_path, dir_names, file_names in os.walk(Path.cwd() / 'files_to_rename'):
        ...
        for file_name in file_names:
            file_name_no_ext = Path(file_name).stem  # Срез имени без разрешения файла
            start = 0
            end = 0
            if name_range:  # Проверки наличия диапазона имени файла
            ...
            if file_extension == old_file_ext:  # проверки условий файла
                if new_file_ext:
                    if new_file_name:
